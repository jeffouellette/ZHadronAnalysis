#ifndef __SystematicTable_h__
#define __SystematicTable_h__
...
class SystematicTable {

  private:
  string name;

  TH1D*** relVarPt = Get2DArray <TH1D*> (nPtZBins-2, numCentBins); // iSpc, iPtZ, iCent
  TH1D*** relVarX  = Get2DArray <TH1D*> (nPtZBins-2, numCentBins); // iSpc, iPtZ, iCent

  public:

  SystematicTable (const char* _name = "systematics") {
    name = _name;
  }
...
////////////////////////////////////////////////////////////////////////////////////////////////
void SystematicTable :: GetRelativeVariations (const string inFileName) {
  ...
  for (short iPtZ = 2; iPtZ < nPtZBins; iPtZ++) { 
    for (short iCent = 0; iCent < numCentBins; iCent++) {
      const char* cent = (iCent == 3 ? "pp" : Form ("CENT%i", numCentBins-iCent-1));

      relVarPt[iPtZ][iCent] = (TH1D*) inFile->Get (Form ("h_hpt_relsys_egs_ZPT%i_%s", iPtZ-2, cent));
      relVarX[iPtZ][iCent] = (TH1D*) inFile->Get (Form ("h_hxh_relsys_egs_ZPT%i_%s", iPtZ-2, cent));
    } // end loop over iCent
  } // end loop over iPtZ
}
...
// raw (unscaled) hadron yields.
////////////////////////////////////////////////////////////////////////////////////////////////
void SystematicTable :: ApplyRelativeVariations (PhysicsAnalysis* a, const bool upVar) {
