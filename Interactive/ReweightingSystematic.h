...
void ReweightingSystematic :: GetRelativeVariations (PhysicsAnalysis* nominal, PhysicsAnalysis* var) {
  ...
  for (short iSpc = 0; iSpc < 3; iSpc++) {
    ...
    for (short iPtZ = 2; iPtZ < nPtZBins; iPtZ++) { 
      ...
      //  for (short iCent = 0; iCent < numCentBins; iCent++) {
      //    h = (TH1D*) nominal->h_trk_pt_dphi[iSpc][iPtZ][iPhi][iCent]->Clone (Form ("h_relVarPt_%s_iPtZ%i_iPhi%i_iCent%i_%s", spc, iPtZ, iPhi, iCent, name.c_str ()));
      //    relVarPt[iSpc][iPtZ][iPhi][iCent] = h;
      //    h->Divide (var->h_trk_pt_dphi[iSpc][iPtZ][iPhi][iCent]);
