...
bool FCalCalibration (const char* directory,
  ...
  TH1D* h_qx_a = new TH1D ("h_qx_a", "q_{x}^{a}", 100, -0.4, 0.4);
  TH1D* h_qy_a = new TH1D ("h_qy_a", "q_{y}^{a}", 100, -0.4, 0.4);
  TH1D* h_qx_c = new TH1D ("h_qx_c", "q_{x}^{c}", 100, -0.4, 0.4);
  TH1D* h_qy_c = new TH1D ("h_qy_c", "q_{y}^{c}", 100, -0.4, 0.4);
  TH1D* h_qxqx_a = new TH1D ("h_qxqx_a", "(q_{x}^{a})^2", 100, 0, 0.4);
  TH1D* h_qxqy_a = new TH1D ("h_qxqy_a", "q_{x}^{a}q_y^{a}", 100, -0.4, 0.4);
  TH1D* h_qyqy_a = new TH1D ("h_qyqy_a", "(q_{x}^{a})^2", 100, 0, 0.4);
  TH1D* h_qxqx_c = new TH1D ("h_qxqx_c", "(q_{x}^{c})^2", 100, 0, 0.4);
  TH1D* h_qxqy_c = new TH1D ("h_qxqy_c", "q_{x}^{c}q_y^{c}", 100, -0.4, 0.4);
  TH1D* h_qyqy_c = new TH1D ("h_qyqy_c", "(q_{x}^{c})^2", 100, 0, 0.4);
  h_qx_a->Sumw2 ();
  h_qy_a->Sumw2 ();
  h_qx_c->Sumw2 ();
  h_qy_c->Sumw2 ();
  ...
  for (int iEvt = 0; iEvt < nEvts; iEvt++) {
    ...
    if (isPbPb) {
      ...
      h_qx_a->Fill (t->fcalA_et_Cos);
      h_qy_a->Fill (t->fcalA_et_Sin);
      h_qx_c->Fill (t->fcalC_et_Cos);
      h_qy_c->Fill (t->fcalC_et_Sin);
      h_qxqx_a->Fill (t->fcalA_et_Cos * t->fcalA_et_Cos);
      h_qxqy_a->Fill (t->fcalA_et_Cos * t->fcalA_et_Sin);
      h_qyqy_a->Fill (t->fcalA_et_Sin * t->fcalA_et_Sin);
      h_qxqx_c->Fill (t->fcalC_et_Cos * t->fcalC_et_Cos);
      h_qxqy_c->Fill (t->fcalC_et_Cos * t->fcalC_et_Sin);
      h_qyqy_c->Fill (t->fcalC_et_Sin * t->fcalC_et_Sin);
    }
    else
